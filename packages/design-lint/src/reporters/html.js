...
export function format(violations, options = {}) {
  ...
  const cleanPct = scanned > 0 ? Math.round((clean / scanned) * 100) : 100;
...
<main>
  ...
  <footer>
    Generated by design-lint. ${baseline
      ? `Measured against a baseline of ${baseline.known.length + baseline.fixed} recorded violation${baseline.known.length + baseline.fixed === 1 ? '' : 's'}.`
...
function renderDrift(drift) {
  ...
  if (!drift.available) return '<p class="empty">No Figma export found, so nothing was compared.</p>';
